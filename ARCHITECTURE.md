...
- **סגור (2026-06-26)**: ספק embeddings ל-RAG — **Voyage AI `voyage-3-lite`** (בבעלות Anthropic). משתמש באותו Anthropic API key שכבר קיים בפרויקט, ללא תלות חיצונית נוספת. עלות: זניחה לנפחי האקתון. אפשרויות שנשקלו ונדחו: OpenAI embedder (דורש key נפרד), Google `text-embedding-004` (תלות ב-GCP credentials נוספים), מודל מקומי HuggingFace (דורש infra). **אילוץ פרטיות**: ה-Cloud Function שמחזיר נתוני משתמש לסוכן (issue #3) חייב לסנן שדות PII (שם, ת"ז, כתובת, טלפון) ולהעביר רק סכומים ותנאי הלוואה — הסינון קורה בצד השרת לפני שהנתונים נכנסים ל-RAG pipeline.